...
<script type="text/template" id="form_preview_template">

        <div class="fixed-box">
            <div id="cover">
                <img src="<%= data.image %>" width="100%" height="237" alt="">
            </div>
            <div id="smallPic">
                <img src="/v2/images/gray_square.jpg" width="90" height="90">
            </div>
            <p class="default-text text-center"></p>

            <h2 class="text-center " style="text-align: center;"><%= data.headline %></h2>
            <% if(data.context_type == 'bullets') { %>
            <% var bullets = JSON.parse(data.bullets); %>

            <ul class="list-unstyled text-center ">
                <% _.each(bullets, function(value,key){  console.log(key,value); %>
                    <li ><%= value %></li>
                <% }); %>
            </ul>
            <% } else { %>
            <p class="text text-center" style="display: none"><%= data.paragraph %></p>
            <% } %>
            <a class="btn btn-primary " href=""><%= data.button_text %></a>
        </div>
</script>
