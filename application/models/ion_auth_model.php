...
//        return (isset($id)) ? $id : FALSE;
    ...
    public function get_logo_by_domain($domain = "")   {
        if ($domain == "")
            return false;

        $r = $this->db->query("SELECT logo FROM users WHERE domain_name='{$domain}' LIMIT 1");
        if ($r->num_rows() > 0)    {
            $r = $r->row_array();
            return $r['logo'];
        } else {
    ...
    public function register_user($username, $password, $email, $additional_data = array()) {
...
//        $ip_address = $this->_prepare_ip($this->input->ip_address());
        ...
        $salt       = $this->store_salt ? $this->salt() : FALSE;
        $password   = $this->hash_password($password, $salt);
        $activationCode = md5($password.$email);
        ...
        if ($additional_data['user_type'] == 'viewer'){
            $data['active'] = 1;
        }
        ...
        return ['user_id' => $id, 'activation_code' => $activationCode];
