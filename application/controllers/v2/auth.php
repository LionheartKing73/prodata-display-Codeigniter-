...
class Auth extends CI_Controller {
	...
	function index()	{
		...
		if ($this->session->userdata("count") > 10)	{
			...
			redirect("http://{$this->data['hostname']}/auth/logout");
			exit;
		}
		...
		if (!$this->ion_auth->logged_in())
		{
			...
			redirect("http://{$this->data['hostname']}/auth/login", 'refresh');
		}
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				if ($this->ion_auth->is_admin())
				{
    				...
    				redirect("http://{$this->data['hostname']}/campclick", 'refresh');
				}
			}
			else
			{
	...
	function change_password()
	{
		...
		else
		{
			...
			else
			{
				...
				redirect("http://{$this->data['hostname']}/auth/change_password", 'refresh');
			}
		}
	}
	...
	function forgot_password()
	{
		...
		else
		{
			...
			else
			{
				...
				redirect("http://{$this->data['hostname']}/auth/forgot_password", 'refresh');
			}
		}
	}
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						redirect("http://{$this->data['hostname']}/auth/reset_password/{$code}", 'refresh');
					}
				}
			}
		}
